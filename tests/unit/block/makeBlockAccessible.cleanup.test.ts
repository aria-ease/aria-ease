...
describe("makeBlockAccessible - cleanup and memory management", () => {
  ...
  it("returns a cleanup function", () => {
    const { cleanup } = makeBlockAccessible({ blockId: "block-container", blockItemsClass: "block-items" });
  ...
  it("allows re-initialization after cleanup", () => {
    const { cleanup: cleanup1 } = makeBlockAccessible({ blockId: "block-container", blockItemsClass: "block-items" });
    cleanup1();

    // Should be able to reinitialize without errors
    const { cleanup: cleanup2 } = makeBlockAccessible({ blockId: "block-container", blockItemsClass: "block-items" });
  ...
  it("handles cleanup on empty block", () => {
    ...
    expect(() => {makeBlockAccessible({ blockId: "empty-block", blockItemsClass: "non-existent-items" });}).not.toThrow();
  });

  it("handles dynamic item addition with re-initialization", () => {
    const container = document.getElementById("block-container")!;
    let { cleanup } = makeBlockAccessible({ blockId: "block-container", blockItemsClass: "block-items" });
    ...
    // Re-initialize
    ({ cleanup } = makeBlockAccessible({ blockId: "block-container", blockItemsClass: "block-items" }));
